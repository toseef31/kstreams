...
groupsRouter.route('/addusergroup').post(function (req, res) {
    ...
    groupModel.update(
        ...
        (result) => {
            ...
            Group.find({ '_id': req.body.selectedGroupId }).populate({ path: 'members', match: { status: { $eq: 1 } } }).exec(function (err, users) {
...
groupsRouter.route("/deletegroup").post(function (req, res) {
    ...
    Group.findByIdAndUpdate(req.body.groupId, { 'status': 0 }).then(
        (result) => {
            var Group = groupModel;
            Group.find({ 'status': 1 }).populate({ path: 'members', match: { status: { $eq: 1 } }, select: { '_id': true, 'email': true, 'username': true, 'status': true } }).exec(function (err, groups) {
                res.send(groups);
            })
            // Group.find({ 'status': 1 }, {}, function (err, groups) {
            //     res.send(groups);
            // })
...
groupsRouter.route("/getgroups").get(function (req, res) {
    ...
    Groups.find({ 'status': 1 }).populate({ path: 'members', match: { status: { $eq: 1 } }, select: { '_id': true, 'email': true, 'username': true, 'status': true } }).exec(function (err, groups) {
        res.send(groups);
    })
